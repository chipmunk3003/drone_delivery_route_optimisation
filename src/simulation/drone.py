...
from database.inventory import update_stock, get_num_available_weights, get_num_available
from database.orders import get_order_by_location, get_order, get_order_items, get_order_location, get_product_quantity_in_order
...
class Drone:
		...
		while not self.tourComplete and not self.paused:
			...
			if distBetween(self.coords, nextPoint) < distInTimeBetween:
				...
				if nextPoint in self.mapDisplay.deliveryPointPins:
					...
					orderId = get_order_by_location(nextPoint[0], nextPoint[1])[0]
					orderWeight, orderId, timeOrderPlaced = get_order(orderId)
			...
			# updates the drone battery
			batteryTakenInTime = self.calcBatteryTaken()  # calls the calcBatteryTaken function
	...
	def getWarehousesNumTaken(self,warehouses):
		...
		# fetches the product weight information and the number of each product available in each warehouse 
		warehouse1Numbers = get_num_available_weights("stock1")
		warehouse2Numbers = get_num_available_weights("stock2")
	...
	def updateTable(self,warehouses):
		warehouse1Stock = get_num_available("stock1")
		warehouse2Stock = get_num_available("stock2")
		...
		for i in range(0,len(self.orderMenu.batchQuantities)):
			...
			# updates the stocks in both warehouses for that product to the new value calculated
			update_stock("stock1", warehouse1Value, i+1)
			update_stock("stock2", warehouse2Value, i+1)
	

	# selects and adds the orders from the orderQueue using first-fit binpacking algorithm
	def addOrdersToBatch(self):
		...
		while pos < len(self.orderMenu.orderQueue):
			...
			if self.orderMenu.batchWeight + weight < 15.9:
				...
				coords = get_order_location(order[0])
				...
				for i in range(0,8):
					...
					num = get_product_quantity_in_order(orderId,i+1)
	...
	def checkWarehousesToVisit(self):
		oneNeeded = False
		twoNeeded = False

		#selects the stock of each item in each warehouse
		warehouse1Numbers = get_num_available("stock1")
		warehouse2Numbers = get_num_available("stock2")
	...
	def checkChargingNeeded(self, trialPath, path, warehousesToVisit):
		...
		if warehousesToVisit =="either":
			...
			for i in range(0,len(trialPath)-1):
				if trialPath[i] == self.mapDisplay.warehouse1Coords:
					...
					orderId = get_order_by_location(trialPath[i][0], trialPath[i][1])[0]
					weight, _ , _ = get_order(orderId)
					#weight = weight[0]
					payload -= weight
		...
		for i in range(0,len(path)-1):
			if path[i] == self.mapDisplay.warehouse1Coords:
				...
				orderId = get_order_by_location(path[i][0], path[i][1])[0]
				weight, _ , _ = get_order(orderId)
				#weight = weight[0]
				payload -= weight
	...
	def startTour(self):				
		...
		self.addOrdersToBatch()
				
		#starts off with the trial cost at a very high number
		trialCost = 1000000000000
		chargingTrialCost = 100000000000
		trialCoordinatePath = []
		warehousesToVisit = self.checkWarehousesToVisit()  #checks which warehouses need to be visited depending on the batch numbers
		...
		chargingNeeded = self.checkChargingNeeded(trialCoordinatePath,self.coordinatePath, warehousesToVisit)
